...
def tackMags(filter):
    ...
    drc_dat = np.genfromtxt(main_dir+'all_drc_'+filter+'_matched_magCuts.dat',names=True)
    flc_dat = np.genfromtxt(main_dir+'all_flc_'+filter+'_matched_magCuts.dat')
    ...
    np.savetxt(main_dir+'flcs_w_drcMAGS_'+filter+'_magCuts.dat',concat,fmt='%1.5f',header=header)
