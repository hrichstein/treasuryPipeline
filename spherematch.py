...
# # To run sphere_match - only needs to be done once (Need to uncomment the sphere_match code from cell above)

# for gg in range(len(names)):
#     i_str=i_fits_name[gg]
#     v_str=v_fits_name[gg]

#     infileI='../source_files_se/sextractor_'+i_str
#     infileV='../source_files_se/sextractor_'+v_str

#     xI = np.loadtxt(infileI, dtype='float')
#     xV = np.loadtxt(infileV, dtype='float')
...
#     idx1, idx2, ds = spherematch(raI, decI, raV, decV, tol=5e-2, nnearest=1)

# #     print(idx1)
# #     print(idx2)

#     outfile = 'sp_match/'+i_str+'_match.txt'
#     np.savetxt(outfile, idx1, fmt='%4i')
#     outfile = 'sp_match/'+v_str+'_match.txt'
