...
match_files_dir = '/Volumes/Spare Data/Hannah_Data/hor1dir1803/dir2503/'
...
def trans_flc2flc(targname,filter):
    ...
    match = np.loadtxt(match_files_dir+"HOR-I_refStars_"+filter+".dat", comments="#")

    weights = np.zeros((len(match)))
    weights.fill(1.0)

    master = np.loadtxt(match_files_dir+"HOR-I_refStars_"+filter+".dat", comments="#")
    ...
    for ff in range(len(jdanUse)):
        ...
        outname = jdanUse[ff]+"_"+targname+"_"+filter+"_t.dat"
