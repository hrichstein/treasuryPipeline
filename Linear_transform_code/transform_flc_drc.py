...
# List of sources already matched to the master frame with x/y in the original frame
    if filter=='f606w':
        ...
        let='i'
    match = np.genfromtxt(main_dir+"flc_"+outpre+"_hor1_1e4.dat", names=True)

# List of matched sources in the master frame
    master = np.genfromtxt(main_dir+"drc_"+outpre+"_hor1_1e4.dat", names=True)
...
# Generate an equal length array containing only 1 for the weights
    ...
    np.savetxt(main_dir+outpre+"_hor1_"+filter+"_xy.dat", new_all, fmt="%1.6f")
