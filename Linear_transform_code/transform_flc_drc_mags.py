...
def trans_flc_drc(outpre,filter):
# List of sources already matched to the master frame with x/y in the original frame
    if filter=='f606w':
        let='v'
    else:
        let='i'
    match = np.genfromtxt(main_dir+"flc_"+outpre+"_hor1_1em2_magCuts.dat", names=True)

# List of matched sources in the master frame
    master = np.genfromtxt(main_dir+"drc_"+outpre+"_hor1_1em2_magCuts.dat", names=True)
...
# Generate an equal length array containing only 1 for the weights
    ...
    new_match, new_all = test_linear(match["xt_1_"+filter], match["yt_1_"+filter], master["x_"+let], master["y_"+let], weights, weights, all["xt_1_"+filter], all["yt_1_"+filter])

    # Save out the two new transformed x/y positions
    np.savetxt(main_dir+outpre+"_hor1_"+filter+"_xy_magCuts.dat", new_all, fmt="%1.6f")
...
for oo in range(len(outpres)):
    for ff in range(len(filters)):
        trans_flc_drc(outpres[oo],filters[ff])
